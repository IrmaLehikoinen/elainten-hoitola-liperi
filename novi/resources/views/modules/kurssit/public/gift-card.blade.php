<x-kurssit::layouts.public title="Osta lahjakortti">
    ...
    <div class="sp-kurssit-page">
        ...
        @if (session('purchase_error'))
            <div style="background:#FEF2F2; border:1px solid #FCA5A5; color:#991B1B; padding:12px 16px; border-radius:8px; font-size:14px; margin:20px 0;">
                {{ session('purchase_error') }}
            </div>
        @endif

        @if (! $onlineEnabled)
            <div style="background:white; border-radius: var(--brand-radius, 16px); padding:28px; margin-top:20px; border:1px solid rgba(42,52,40,0.08);">
                <p style="color: var(--brand-text);">Lahjakortin ostaminen verkossa ei ole tällä hetkellä käytössä.</p>
            </div>
        @else
            <div style="background:white; border-radius: var(--brand-radius, 16px); padding:24px 28px; box-shadow:0 1px 4px rgba(42,52,40,0.05); border:1px solid rgba(42,52,40,0.08); margin:20px 0 24px;">
            ...
            <div style="background:white; border-radius: var(--brand-radius, 16px); padding:28px; box-shadow:0 1px 4px rgba(42,52,40,0.05); border:1px solid rgba(42,52,40,0.08);">
                <form method="POST" action="{{ route('kurssit.public.gift-card.store') }}" style="display:flex; flex-direction:column; gap:14px; max-width:420px; margin:0 auto;">
                    ...
                    <p style="font-size:13px; color: var(--brand-text); opacity:0.7; margin:-8px 0 0;">Jos täytät nämä, kortti lähetetään suoraan hänelle. Muuten se lähetetään sinulle.</p>
