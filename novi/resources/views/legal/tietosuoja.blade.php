...
<head>
    ...
    <title>Tietosuojaseloste — {{ $companyRecord->name ?? 'Ajanvaraus' }}</title>
    <style>
        body {
...
<body>
    ...
    <h2>3. Rekisterin nimi</h2>
        <p>{{ $companyRecord->settings['official_name'] ?? $companyRecord->name ?? 'Yrityksen' }} asiakas- ja {{ $companyRecord->industry === 'kurssit' ? 'ilmoittautumis' : 'varaus' }}rekisteri</p>

    <h2>4. Henkilötietojen käsittelyn tarkoitus ja oikeusperuste</h2>
    @if ($companyRecord->industry === 'kurssit')
        <p>Henkilötietoja käsitellään kurssi-ilmoittautumisten ja lahjakorttien myynnin toteuttamiseksi, asiakassuhteen hoitamiseksi sekä laskutusta varten. Käsittelyn oikeusperusteena on asiakkaan ja rekisterinpitäjän välinen sopimus (ilmoittautuminen/lahjakortin osto) sekä lakisääteinen velvoite (kirjanpitolaki, laskutustietojen säilytys).</p>
    @else
    ...
    @if ($companyRecord->industry === 'kurssit')
        <ul>
            <li>Asiakkaan nimi, puhelinnumero, sähköpostiosoite</li>
            <li>Kurssi-ilmoittautumistiedot: ilmoittautumisajankohta, kurssi, maksutiedot</li>
            <li>Lahjakorttitiedot: ostajan ja vastaanottajan nimi ja sähköposti, mahdollinen viestitervehdys, summa</li>
    ...
    <p>Tiedot saadaan asiakkaalta itseltään varauksen tai ilmoittautumisen yhteydessä (verkkolomake tai henkilökunnan kirjaamana).</p>
    ...
    <p>Asiakastietoja säilytetään niin kauan kuin asiakassuhde on voimassa. Laskutustiedot säilytetään kirjanpitolain edellyttämät 6 vuotta.</p>
